...
void Qtilities::UnitTests::qti_private_FunctionCallAnalyzer::count(const QString& function_name) {
    int current_value = 0;
...
void Qtilities::UnitTests::qti_private_FunctionCallAnalyzer::clear(const QString& function_name) {
    call_counts.remove(function_name);
}

void Qtilities::UnitTests::qti_private_FunctionCallAnalyzer::clearAll() {
    call_counts.clear();
}

void Qtilities::UnitTests::qti_private_FunctionCallAnalyzer::logCallCount(const QString& function_name) const {
    if (call_counts.count() > 0) {
        if (call_counts.contains(function_name)) {
