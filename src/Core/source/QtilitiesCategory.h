...
namespace Qtilities {
    namespace Core {
        ...
        public:
            CategoryLevel(QDataStream& stream, Qtilities::ExportVersion version) : IExportable(), IObjectBase() {
                QList<QPointer<QObject> > import_list;
                setExportVersion(version);
                importBinary(stream,import_list);
            }
            CategoryLevel(const QString& name = QString()) : IExportable(), IObjectBase() {
                d_name = name;
            }
            CategoryLevel(const CategoryLevel& ref) : IExportable(), IObjectBase() {
                d_name = ref.d_name;
            }
...
*/
        ...
        public:
            ...
            QtilitiesCategory(QDataStream &ds, Qtilities::ExportVersion version) : IExportable(), IObjectBase() {
                QList<QPointer<QObject> > import_list;
                setExportVersion(version);
                importBinary(ds,import_list);
            }
            QtilitiesCategory(const QtilitiesCategory& category) : IExportable(), IObjectBase() {
